...
## 2026-07-22 排座界面视觉层级整改

本轮依据界面截图审查清单完成首轮视觉调整：

- 排座路由隐藏重复的全局页签，减少侧栏、面包屑和页签的三重导航噪声。
- 六个排座列表页统一增加页面标题、说明文字、筛选面板边界和表格最小高度，改善信息层级与留白节奏。
- 座位方案详情页按「座位表—导出—方案操作」重新组织操作区，导出动作合并到下拉菜单，保存调整和确认方案保持主要视觉权重。
- 详情页评分侧栏增加宽度弹性与表格列宽约束；座位表增加溢出保护和 1400px 响应式断点。
- 编辑、导出、清空座位等非关键动作改用中性色，减少蓝绿红高饱和颜色的同时保留状态语义。
...
结果：前端 Vite 生产构建成功。真实登录态浏览器复核待后续补充。

## 2026-07-22 静青教务设计系统

已完成：

- 新增 `docs/DESIGN-SYSTEM.md`，确定慧排座使用面向老师的“静青教务”设计风格。
- 明确青绿色品牌主色、低刺激中性色、语义状态色、字号、间距、圆角和动效规则。
- 明确首页、列表页、表单和排座详情页结构；排座详情采用“座位图＋单侧检查面板”两栏结构，避免 1366px 屏幕下三栏挤压座位图。
- 明确座位锁定、冲突、空座、过道和不可用状态的非颜色表达。
- 明确先做代表页面视觉验收，再扩展全站，不在本轮直接修改前端实现。

验证：

- 已核对设计规范包含可执行数值、技术边界、实施顺序和验收标准。
- 本轮仅新增设计文档和进度记录，无需执行前后端构建。

阻塞解除（2026-07-22）：

- 内置图片服务已恢复，无需 OPENAI_API_KEY 即可生成视觉样稿。
- 待按淡青绿配色更换方案更新设计 token 与前端主题后，再生成视觉样稿做目视验收。
