## 2026-06-30 新学期复制创建实现验收

已完成：

- 班级管理列表已新增“新学期复制”入口，支持目标班级名称、学年、学期和四项复制开关。
- 后端已实现新学期复制事务：默认复制学生、学生关系、排座规则和默认教室布局，并明确不复制考试、成绩、座位方案、分配结果和方案评分。
- 真实验证已通过：`mvn -DskipTests compile`、`mvn -Dtest=SeatClassCopySupportTest test`、`node --test src/views/seating/class/__tests__/newTermCopyState.test.js`、`npm run build:prod`。
...
- 2026-06-30 已重新执行座位方案详情页动作状态相关单测与前端 `npm run build:prod`，当前均可通过；昨晚出现的 Vite `index.html` 绝对路径 emitted chunk 报错本次未再复现。
- 2026-06-30 已在真实浏览器中回归启用方案详情页高频链路：确认按钮已隐藏，三类导出按钮可用；`R7C10` 已完成批量锁定、保存成功、再批量解锁并保存恢复，过程中按钮状态与成功提示正常，控制台未出现新的警告或错误。
...
4. P2：新学期复制创建已实现并验证，模板能力继续作为复制流程内的来源选择。
...
3. 在 `P1` 稳定后，再进入阶段 9 `P2`，推进新学期复制创建能力。
